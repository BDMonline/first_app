...
begin
    ...
    def show
        ...
        if @error 
            flash[:success] = "There was a problem with this question. Please address the issues below."
            @question.destroy
            render 'edit'
 	    else
 ...
 @testtext=calculate('-2-1/(2-4*(2+ln(2)))')
        ...
        if @question.update_attributes(params[:question])
            construct
            if @error
                flash[:failure] = "There was a problem with this question"
                render 'edit'
            else
                flash[:success] = "The website could not detect a problem with this question. But what does a website know? Please check thoroughly and refresh the page if appropriate to explore the effect of different parameter choices."
                render 'show'
            end
        else
            flash[:failure] = "There was a problem with this question"
